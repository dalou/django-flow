...
def store_groups_in_session(sender, user, request, **kwargs):
    ...
    This is required by . since fetching groups accesses the database, which is a blocking
    operation and thus not allowed from within the websocket loop.
    """
    if hasattr(user, 'groups'):
        request.session['flow_redis_ws:memberof'] = [g.name for g in user.groups.all()]
