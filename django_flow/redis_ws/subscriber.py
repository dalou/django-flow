...
from .redis_store import RedisStore, SELF
...
class RedisSubscriber(RedisStore):
    ...
    def set_pubsub_channels(self, request, channels):
        ...
        """
        facility = request.path_info.replace(settings.FLOW_WS_URL, '', 1)
    ...
    def get_user_pk_for_channel(self, channel_name, subscribed=False):
        """
        Extract user_pk from user channel given by the channel name
        """
        user_pk = None
        if not subscribed or (self._subscription and self._subscription.subscribed):
            for channel in self._subscription.channels.keys():
                user_pk = re.match(r"%s\:user\:(\d+)\:%s" % (settings.FLOW_WS_PREFIX, channel_name), channel ) #ws:user:1:flow
                if user_pk:
                    user_pk = user_pk.group(1)
                    if user_pk:
                        user_pk = int(user_pk)
        return user_pk
